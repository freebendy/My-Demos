...
class StackOverflow extends StackOverflowInterface with Serializable {
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(QID, Iterable[(Question, Answer)])] = {
    val qRdd = postings.filter(p => p.postingType == 1).map(p => (p.id, p))
    val aRdd = postings.filter(p => p.postingType == 2).map(p => (p.parentId.get, p))
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] = {
    ...
    val clusterAvg = vectors.map(p => (findClosest(p, means), p)).groupByKey().mapValues(averageVectors).collect()
    for ((x, y) <- clusterAvg) {
      newMeans.update(x, y)
    }
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(LangIndex, HighScore)]): Array[(String, Double, Int, Int)] = {
    ...
    val median = closestGrouped.mapValues { vs =>
      val mostCommonLang = vs.groupBy(_._1).mapValues(_.size).maxBy(_._2)
      ...
      val medianScore: Int    = {
        ...
        val midIndex: Int = sortedScores.size / 2
        sortedScores.size % 2 match {
          case 0 => (sortedScores(midIndex) + sortedScores(midIndex - 1)) / 2
          case 1 => sortedScores(midIndex)
        }
      }
